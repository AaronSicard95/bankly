...
const patchUserSchema = require("../updateUserSchema.json");
const ExpressError = require('../helpers/expressError');
const { authUser, requireLogin, requireAdmin, requireAdminOrSameUser } = require('../middleware/auth');
...
router.get('/', authUser, requireLogin, async function(req, res, next) {
  try {
    ...
    //FIXES BUG 2
    let retUsers = users.map(m=>({username:m.username,first_name:m.first_name,last_name:m.last_name}));
    return res.json({ users: retUsers });
  } catch (err) {
    return next(err);
  }
}); // end
...
router.patch('/:username', authUser, requireLogin, requireAdminOrSameUser, async function(
  req,
  res,
  next
) {
  try {
    ...
    //FIXES BUG 4
    const val = jsonschema.validate(fields, patchUserSchema);
    if (!val.valid) {
      let listOfErrors = val.errors.map(error => error.stack);
      let error = new ExpressError(listOfErrors, 401);
      return next(error);
    }
